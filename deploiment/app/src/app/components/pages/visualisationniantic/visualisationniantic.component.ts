...
@Component({
  selector: 'app-visualisationniantic',
  templateUrl: './visualisationniantic.component.html',
  styleUrls: ['./visualisationniantic.component.css'],
})
export class VisualisationnianticComponent implements OnInit {
  ...
  searchyoutube() {
    ...
    this.CompetitorPostsService.fetchrateyoutube(
      ...
      (res1: rates) => {
        ...
        console.log(this.competitor_rates1);
  ...
  getalexa(): void {
    this.competitorService.getalexa().subscribe((res) => {
      ...
      for (let pas = 0; pas < this.apialexa.length; pas++) {
        if (
          this.apialexa[pas]['competitor'] == 'niantic' ||
  ...
  getdonnegeneral(): void {
    this.competitorService.getcompetitors().subscribe((res) => {
      ...
      for (let pas = 0; pas < this.apiCompititors.length; pas++) {
        if (
          this.apiCompititors[pas]['competitor'] == 'niantic' ||
  ...
  getyoutubechanel() {
    this.competitorService.getyoutubechaine().subscribe((res) => {
      ...
      for (let pas = 0; pas < this.apiyout.length; pas++) {
        if (
          this.apiyout[pas]['competitor'] == 'niantic' ||
  ...
  getrev() {
    this.competitorService.getreviws().subscribe((res) => {
      ...
      for (let pas = 0; pas < this.apireviwes.length; pas++) {
        if (
          this.apireviwes[pas]['competitor'] == 'niantic' ||
